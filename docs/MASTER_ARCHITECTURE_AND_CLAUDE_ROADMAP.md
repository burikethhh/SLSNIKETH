...
1. **CEO Admin Authentication** (migrated 2026-09-04: the shared master key is gone):
   - CEO accounts in `cloud_ceo_accounts` (`ceo_email`, Argon2id `password_hash`, `display_name`). First CEO registers via `POST /api/v1/auth/ceo-register` (open bootstrap on a fresh DB only; afterwards CEO-authenticated creation), then signs in via `POST /api/v1/auth/ceo-login` (validated email, min-8-char password, tight per-account + per-IP rate limits).
   - Verified on all `/api/v1/admin/*`, `/api/v1/licenses/*`, `/api/v1/gyms/*`, and `/api/v1/remote/*` endpoints via `Authorization: Bearer ceo:<email>`, checked against the accounts table.
   - **Audit fix (2026-09-03)**: `POST /api/v1/auth/ceo-login`, `POST /api/v1/owner/auth/login`, and `POST /api/v1/owner/auth/register` are now rate-limited (`cloud/src/rate_limit.rs`) to slow down credential brute-forcing — 5 attempts/15min per (IP, email) plus 30/15min per IP for CEO login, 8/10min per (IP, email) plus 30/10min per IP for owner login, 5/hour per IP for registrations. Rejected requests get `429` with a `Retry-After` header; successful logins reset the counter for that key.
