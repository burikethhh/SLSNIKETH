...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let app = Router::new()
        ...
        // Phase B: tailgate Security Console (fleet incidents + per-branch policy)
        .route("/api/v1/admin/incidents", get(routes::admin_list_incidents))
        .route("/api/v1/admin/incidents/:id/ack", post(routes::admin_ack_incident))
        .route("/api/v1/admin/gyms/:gym_id/tailgate", post(routes::admin_set_gym_tailgate))
        ...
        // Phase C: owner tailgate incidents (franchise-scoped feed + ack)
        .route("/api/v1/owner/incidents", get(routes::owner_list_incidents))
        .route("/api/v1/owner/incidents/:id/ack", post(routes::owner_ack_incident))
